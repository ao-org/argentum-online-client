    ...
    #Else
        Select Case PacketId
            ...
            Case ServerPacketID.eShowPickUpObj
                ...
                #End If
            Case ServerPacketID.eNpcMultiTileInfo
                Call HandleNpcMultiTileInfo
            Case Else
                ' Invalid Message
        End Select
    #End If
...
Private Sub HandleCharacterCreate()
    ...
    With charlist(charindex)
        ...
        ' Initialize as single-tile by default
        .IsMultiTile = False
        .TileWidth = 1
        .TileHeight = 1
        .BaseTileX = x
...
Private Sub HandleNpcMultiTileInfo()
    On Error GoTo HandleNpcMultiTileInfo_Err
    
    Dim charindex As Integer
    Dim TileWidth As Byte
    Dim TileHeight As Byte
    
    charindex = Reader.ReadInt16()
    TileWidth = Reader.ReadInt8()
    TileHeight = Reader.ReadInt8()
    
    If charindex > 0 And charindex <= UBound(charlist) Then
        With charlist(charindex)
            .IsMultiTile = True
            .TileWidth = TileWidth
            ...
            Call MarkMultiTileNpcOnMap(charindex)
        End With
    End If
    
    Exit Sub
HandleNpcMultiTileInfo_Err:
    Call RegistrarError(Err.Number, Err.Description, "Protocol.HandleNpcMultiTileInfo", Erl)
End Sub

Private Sub MarkMultiTileNpcOnMap(ByVal charindex As Integer)
    On Error GoTo MarkMultiTileNpcOnMap_Err
    ...
    With charlist(charindex)
        If Not .IsMultiTile Then Exit Sub
    ...
    Exit Sub
MarkMultiTileNpcOnMap_Err:
    Call RegistrarError(Err.Number, Err.Description, "Protocol.MarkMultiTileNpcOnMap", Erl)
End Sub
