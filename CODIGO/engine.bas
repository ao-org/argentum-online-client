...
Public Sub render()
    ...
    Dim renderW       As Long
    Dim renderH       As Long
    renderW = frmMain.renderer.ScaleWidth
    renderH = frmMain.renderer.ScaleHeight
    ...
    If FadeInAlpha > 0 Then
        Call Engine_Draw_Box(0, 0, renderW, renderH, RGBA_From_Comp(0, 0, 0, FadeInAlpha))
...
Public Sub Mascota_Render(ByVal charindex As Integer, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer)
    ...
    Dim halfW      As Long
    Dim halfH      As Long
    halfW = frmMain.renderer.ScaleWidth \ 2
    halfH = frmMain.renderer.ScaleHeight \ 2
    'target_charindex in pixels on render:
    target_x = halfW - ((UserPos.x - AddtoUserPos.x) - charlist(charindex).Pos.x) * 32 + charlist(charindex).MoveOffsetX
    target_y = halfH - ((UserPos.y - AddtoUserPos.y) - charlist(charindex).Pos.y) * 32 + charlist(charindex).MoveOffsetY
    ...
    If dist_x > 40 Then
        mascota.PosX = mascota.PosX + (dir_vector.x / halfW) * timerElapsedTime / 1000 * dist * 3  ' 256 como constante no le da aceleraciï¿½n.
        isAnimated = 1
    End If
    mascota.PosX = mascota.PosX - LastOffset2X
    If dist_y > 40 Then
        mascota.PosY = mascota.PosY + (dir_vector.y / halfH) * timerElapsedTime / 1000 * dist * 3
        isAnimated = 1
    End If
